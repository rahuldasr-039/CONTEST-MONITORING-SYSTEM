...
def get_lc_light(handle):
    ...
    try:
        ...
        rank = resp['data']['matchedUser']['profile']['ranking']
...
def get_detailed_stats(user):
    ...
    if user.cf_handle:
        try:
            ...
            solved_dates = {datetime.fromtimestamp(s['creationTimeSeconds']).strftime('%Y-%m-%d') for s in subs if s['verdict'] == 'OK'}
            ...
            for i in range(6, -1, -1):
                ...
                unique_solved_count = len({f"{s['problem']['contestId']}{s['problem']['index']}" for s in subs 
                                         if s['verdict'] == 'OK' and d_start <= s['creationTimeSeconds'] < d_end})
                cf_history.append({'date': d_obj.strftime('%Y-%m-%d'), 'count': unique_solved_count})

            solved_set = {f"{s['problem']['contestId']}{s['problem']['index']}" for s in subs if s['verdict'] == 'OK'}
            
            stats['Codeforces'] = {
                'solved': len(solved_set),
                'total_questions': CF_TOTAL_GLOBAL,
                'solved_percentage': round((len(solved_set) / CF_TOTAL_GLOBAL) * 100, 2),
                'today_count': len({f"{s['problem']['contestId']}{s['problem']['index']}" for s in subs if s['verdict'] == 'OK' and s['creationTimeSeconds'] >= start_of_today}),
                'streak': cf_streak,
                'history': cf_history,
                'rating': info.get('rating', 'Unrated'),
                'rank': info.get('rank', 'Unrated'),
                'last_contest': next((r['contestName'] for r in requests.get(f"https://codeforces.com/api/user.rating?handle={user.cf_handle}").json().get('result', [])[::-1]), "N/A"),
                'error': False
            }
    ...
    if user.lc_handle:
        ...
        try:
            ...
            user_data = resp['data']['matchedUser']
            ...
            last_c = next((c['contest']['title'] for c in reversed(resp['data']['userContestRankingHistory']) if c['attended']), "None")

            stats['LeetCode'] = {
                'solved': solved,
                'total_questions': LC_TOTAL_GLOBAL,
                'solved_percentage': round((solved / LC_TOTAL_GLOBAL) * 100, 2),
...
def check_contest_participation(user, contest_name, platform):
    ...
    if platform == "LeetCode" and user.lc_handle:
        ...
        try:
            ...
            for rec in r['data']['userContestRankingHistory']:
                if rec['attended']:
        ...
        try:
            ...
            if r['status'] == 'OK':
                for contest in r['result']:
                    actual_title = contest['contestName'].lower()
